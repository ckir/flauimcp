...
- **`wait_for until=gone` means gone from the TREE, not "off screen".** An element with `IsOffscreen=true` still satisfies it instantly; one merely past the window edge no longer does (it used to, and reported a live element destroyed). Pass `includeOffscreen:true` to wait on either. Same flag now also reaches `until=valueEquals`.
