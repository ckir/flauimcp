...
- **Occlusion-aware capture.** A window that something else is covering can now be captured through
  `PrintWindow` instead of photographing whatever sits on top of it. `desktop_screenshot` metadata gains
  `captureMethod` (`printWindow` or `screenScrape`) and `captureWarnings`, six codes that each carry their
  own recourse text: `UniformCanvas`, `DesktopCanvasUniform`, `ElementCanvasUniform`, `PopupsNotRendered`,
  `ScrapeFallbackTargetUnresponsive`, `ScrapeFallbackTargetChanging`. Not every framework renders under
  `PrintWindow`, which is exactly what the warnings exist to tell you: a uniform-canvas detector catches
  the blank-surface case, a per-HWND circuit breaker stops retrying an app that never renders, and the
  coordinator falls back to a screen scrape rather than handing back an empty image.

- **The server now states its own operating contract at connect**, so a client that reads MCP server
  instructions gets the framing without any skill installed. The payload is split in two: a
  client-agnostic **Core** carried in `InitializeResult.instructions`, and an addendum with the
  Claude-Code-specific tool-loading recipe delivered separately through the SessionStart hook. The Core
  deliberately names no client's tool-discovery mechanism. `flaui-mcp status` reports it as
  `Server instructions: advertised at connect (N chars)` -- **advertised**, not delivered, because a
  client that drops the field cannot be detected from this side.
  Measured, and worth knowing before relying on it: agy parses `instructions` and does not surface it, so
  for that client the driving skill's frontmatter remains the only channel.

- **`desktop_list_terminal_tabs`** enumerates a Windows Terminal tab strip **without switching tabs** --
  no `Select` call -- so you can find which tab holds what before deciding to read one. Reading a
  background tab still switches to it; listing no longer does.

- **`desktop_wait_for` gains `includeOffscreen`**, which also sharpens `until=gone`: an element UIA reports
  as offscreen satisfies `gone` immediately, while `includeOffscreen:true` keeps such elements in scope so
  the wait holds out for real removal. A timeout on `exists`/`enabled` caused by geometry now explains
  itself rather than just expiring. `desktop_wait_for_stable` can be scoped by `ref`.

- **Internal tooling:** `scripts/release.ps1` gains `-NoPush`, so a version can be stamped locally --
  gate, changelog, version bump, commit and tag -- without publishing. No effect on the shipped server.
