...
surface while having been told nothing about what it is for, and - the part that actually bites - nothing
about what it must **not** do.

**The three channels, all verified by reading the source, and what each is scoped to:**

| # | channel | where | scope |
|---|---|---|---|
| 1 | `McpServerOptions.ServerInstructions = ActivationPayload.Core` | `src/FlaUI.Mcp.Server/Install/McpServerConfiguration.cs:17` | the connecting client's system prompt |
| 2 | the SessionStart hook's `additionalContext` (Core + Addendum) | `src/FlaUI.Mcp.Server/Install/ActivationPayload.cs:84-90`, generated by `PluginArtifactWriter` | **once per session, at session start** |
| 3 | the driving skill's `description:` frontmatter | `plugins/flaui-mcp/skills/driving-flaui-mcp/SKILL.md` | wherever the Skill tool is offered |

Channel 2 is session-scoped by construction - `InstallStatus.cs:103` records that *"Claude Code loads
hooks only at client startup"* - so a subagent spawned mid-session is downstream of the only firing it
ever gets. Channel 1 is the client's prompt, not the subagent's. Channel 3 is the one that could plausibly
reach a subagent, and only if the subagent is given the Skill tool and goes looking.

**Why the prohibition matters more than the capability here.** `ActivationPayload.Core` leads with
*"Never ask the user to look at, read, or click inside a desktop app on your behalf, and do not infer UI
state indirectly from process lists."* That line exists because agents reach for those workarounds by
...
- **Put the prohibition where the tools are.** The one place guaranteed to travel with a tool is the tool
  itself: its description. A short prohibition on the read-only entry points costs tokens on every
  listing but cannot be scoped away. This is the only option that does not depend on the harness.
- **Have the dispatcher carry it.** Make the driving skill instruct the dispatching agent to paste the
  Core payload into every subagent dispatch. Cheap, and it fails silently the moment someone dispatches
...
session memory flagged *"still needs its own ROADMAP item"* for a day before being filed here, which is
its own small instance of the problem.

### 30. `desktop_get_grid_cell` indexes the REALIZED set, and the wake mechanism it was found beside is now contested — MEASURED 2026-09-09
...
Measured on a 500-file Explorer folder with the viewport scrolled to ~450 — the realized set was 21 rows
(`item-000` still anchored and offscreen, plus `item-449..468`):

| call | returns |
|---|---|
| `row 0` | `item-000.txt` |
| `row 1` | **`item-449.txt`** — not `item-001.txt`, and no error |
| `row 499` | `GridCellOutOfRange, "outside the 21x4 grid"` |

The index addresses the REALIZATION WINDOW, not the list; `RowCount` tracks realized rows (19 at the top,
