...
- **Window scope: RETRY if there are masks and refuse only on exhaustion; capture and warn only if
  ...
  - **mask set NON-EMPTY → RETRY, then FALL BACK to the scrape on exhaustion.** ⚠ **This said REFUSE
    until 2026-08-21**; see the §4 table's struck row for why the operator reversed it.** Re-take the geometry-and-capture pair on
...
| ~~resize with a NON-EMPTY mask set, after retry exhaustion~~ **— NO LONGER A REFUSAL** | — | ⚠ **REMOVED BY OPERATOR DECISION, 2026-08-21**, reversing part of ratification item 2 because item 2's own premise had become false. The refusal existed because the mask rects were measured at `W1` time and painted onto a scrape taken later. The fallback now performs a **FRESH desktop mask walk at capture time** (§2.5's sibling change, plan panel round 6), so image and masks are acquired together. Both scopes now FALL BACK with `scrapeFallbackTargetChanging`. Keeping the refusal cost a total outage for any animating window holding one redactable field — windows today's tool captures perfectly well. **What still refuses is the BOOKEND mismatch below**, where the mask set is PROVEN to have moved and no re-walk can fix a frame already composed |
