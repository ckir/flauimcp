...
| resize with a NON-EMPTY mask set, **after retry exhaustion, EITHER SCOPE** | `RedactionUnmaskable` | this is precisely that code's meaning — the redacted regions cannot be reliably located — and it is the code SP4 established for "refuse rather than return an under-masked image". **Reached only on exhaustion** since the operator ratification of 2026-08-21; a single mismatch retries. ⚠ **This row said "window-scope" until 2026-08-21**, while the plan had refused ELEMENT scope too since its panel round 4 — a resize reflows the window's layout regardless of which scope asked. A table asserting completeness that scopes a row more narrowly than the code is the same defect as omitting a row |
