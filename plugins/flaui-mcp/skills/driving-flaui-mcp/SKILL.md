...
<!-- Machine-owned region (flaui-curate). Do not hand-edit. HARD CAP: ≤ 120 lines between the markers. -->
- **Find a CLI app in Windows Terminal — never ask the human instead.** An unresponsive agent/service is a PERCEPTION task: one shallow `desktop_list_windows` finding no top-level window is NOT an answer — a CLI agent lives in a TAB, and the WT window's `Hint` says so. Filter `ProcessName == WindowsTerminal`, then `desktop_list_terminal_tabs <wt>` lists every tab in ONE cheap call — prefer it over `desktop_find controlType:"TabItem"`, whose `max` defaults to 20 and whose ordinal is not guaranteed to be a `tabIndex`.
  **A tab title is a HINT whose worth depends on the SHELL:** `cmd.exe` REWRITES its title to name each child
  (`cmd.exe - python  -i`) so it informs; `powershell` does NOT - a Python REPL hid behind a `powershell.exe`
  tab. Read every generic-titled tab. `read_terminal_tab` switches+restores and is the ONLY safe WT buffer read:
  `get_text` by ref fails `AmbiguousMatch` (WT names the tab header and the body identically). *(2026-09-09)*
- **Closing a window: identify by ProcessName + title, and never judge the close on its first sample.** A title substring ALONE matches unrelated apps — an editor, browser tab or file manager showing a FILE whose name contains your marker matches exactly as well as the window you meant, so an automated cleanup can close the human's own work and it looks like a successful tidy-up; conjoin `ProcessName` from the SAME `desktop_list_windows` row (a distinctive case-sensitive marker is NOT enough on its own). And WT tears down asynchronously while it reaps its child consoles, so a window you closed stays enumerable for SECONDS — poll ~15s, treat "still listed" as UNDECIDED rather than as failure, and never escalate on the first sample. *(live 2026-07-30)*
  A WT window may refuse EVERY window-level close (`close_window`, and `invoke` or a synthetic click on the
  titlebar Close, all return ok and do nothing) - invoke each TAB's own "Close Tab" (`automationId CloseButton`)
  instead, the last tab takes the window; and a click on a NON-foreground window is eaten as ACTIVATION, so
  focus first. *(2026-09-09)*
- **Win11 context menu items ARE in UIA - the old rule here was WRONG.** Snapshot shows 1 empty `PopupHost` Pane
  only because the `Pane @{0,0,0,0}` -> `Window "Popup"` (aid `OverflowPopup`) chain is ZERO-BOUNDS and the
  off-screen cull drops the subtree: `includeOffscreen:true` -> all 35 nodes, `interactiveOnly:false` alone -> 1.
  Use `desktop_find controlType:MenuItem` - all 16 at DEFAULTS, real automationIds + Invoke; a bare `desktop_key`
  chord (window, no ref) misses the menu, hits the owner's Pane, and still returns `ok:true`. *(2026-09-09)*
- **`foregroundGained:true` does NOT mean usable, and a minimized tree is NOT one node.** `desktop_focus_window`
...
- **A watch's FIRST `drain_events` proves nothing; heavy coalescing is INVISIBLE in `droppedCount`.**
  ...
  once, cross-check a snapshot, and `scope` the watch up front if you need WHERE. *(2026-09-09)*
- Common file dialog ("Save as"/"Open"): the filename box is buried under ~20+ `System.ItemNameDisplay` file-list
  cells — target it by name `"File name:"` / automationId `1001`; recipe: `set_value` full path → click Save
  (automationId `"1"`). Modal file dialogs ARE enumerable top-level windows.
- Rename a file (F2) opens a SEPARATE top-level window (title=filename) hosting the edit — `set_value` its
  `System.ItemNameDisplay` "Name" Edit → `Enter`. `explorer.exe <folderpath>` spawns a real new window+pid;
  launching Notepad instead TABS into the existing process (LaunchTimeout, but a same-pid tab-window appears).
- Explorer address bar reads empty via `get_text` in breadcrumb mode — read the path from the breadcrumb
  SplitButton segments (lease-exempt), or `Ctrl+L` to morph it to an editable Edit that returns the full path
  (`Esc` restores).
- `desktop_clipboard_get` returns `""` for BOTH empty AND non-text (file/image) clipboards (no format signal);
  `desktop_paste_text` refuses `ClipboardHoldsNonText` (`forceOverwriteClipboard` to override).
- New Win11 Notepad: unsaved tabs show in the `TabItem` **NAME** (trailing `". Modified."`); its child `Text` holds
  the clean title, so the child alone never reveals dirty state. The window title's leading `*` works too. *(live
  2026-07-15)*
- Shell surfaces (Notification Center `Win+N`; Start/Search `Ctrl+Esc` — bare `Win` isn't a valid chord; likely
...
- **WPF trees are FLATTER and DENSER than the XAML implies.** Layout panels (`Grid`/`StackPanel`/`Canvas`/
  ...
  child `Text` peer at the button's rect: one button is TWO nodes. *(verified live 2026-07-28)*
- **The lease's REMAINING time is a precondition, not just its existence.** `desktop_input_status` returns
  `secondsRemaining`, not merely active/locked — compare it against the WHOLE operation BEFORE starting, because a
  lease that lapses mid-run turns lease-guarded work from asserting into SKIPPING, which most runners report as
  success. And to establish *which session you are actually in* (see the RDP-vs-console rule in the manual above),
  read the STATE column from `qwinsta` — never `$env:SESSIONNAME`, which is a per-process snapshot frozen at launch
  and was seen still reporting `RDP-Tcp#0` well after the session had moved to the physical console — and a HUMAN's
  earlier report of which session they are in goes stale exactly the same way, so MEASURE rather than ask. Scope it
  correctly too: UIA and window enumeration work fine over RDP, only `SendInput` does not — an Active console row
  with rdp-tcp merely Listening means synthetic input WILL deliver. *(live 2026-07-28/29/30)*
- **`wait_for until=gone` splits two kinds of not-visible.** An element UIA reports `IsOffscreen=true` still
  satisfies it instantly; one merely laid out past the window edge no longer does (it used to, and reported a live
  element destroyed). `includeOffscreen:true` makes `gone` STRICTER (offscreen elements stay in the tree, so it
  waits for real removal) — and `until=valueEquals` never applied the window-edge cull at all, so it reaches
  past-the-edge elements even without the flag.
- **"No window" is not "not running" — and the process list is how you tell those apart, never how you read the
  screen.** A GUI app closed to tray/hidden owns no top-level window, so `desktop_list_windows` correctly returns
  NOTHING and that is indistinguishable from a dead process or a broken tool. Cross-check the OS process list and
  read `MainWindowTitle`: alive with an EMPTY `MainWindowTitle` means "running, no top-level window". That
  disambiguation is the one sanctioned use of a process list — never infer what is ON the screen from it. *(live
  2026-07-29, Electron)*
- **UIA-sees-it-but-OCR-does-not is an OCCLUSION signature — never a capture-pipeline or redaction fault.** Windows
  refuses a foreground change to a process with no recent REAL human input, so an unattended Desktop test that
  focuses a window and then OCRs that window's screen RECT photographs whatever occludes it: `find_text` returns
  zero matches while UIA resolves the very same element fine. It reproduced deterministically twice in isolation,
