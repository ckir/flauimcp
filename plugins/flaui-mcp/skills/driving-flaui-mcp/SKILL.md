...
- **`wait_for until=gone` splits two kinds of not-visible.** An element UIA reports `IsOffscreen=true` still satisfies it instantly; one merely laid out past the window edge no longer does (it used to, and reported a live element destroyed). `includeOffscreen:true` waits on either — and `until=valueEquals` never applied the window-edge cull at all, so it reaches past-the-edge elements even without the flag.
