...
public sealed class InteractionTools
{
    ...
    [McpServerTool(Destructive = true), Description("Invoke (activate) an element by ref via its UIA InvokePattern (e.g. click a button). If it opens a modal you get ActionBlockedPending — snapshot to see the dialog, then act on it. UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Set keyboard focus to an element by ref (UIA Focus). Often a prerequisite that reveals lazy-loaded content or enables downstream controls. UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Set an element's value by ref via UIA ValuePattern (fast text/value set; focuses first). ElementNotActionable if read-only, PatternUnsupported if no ValuePattern (no synthetic typing this phase). UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Toggle an element by ref via UIA TogglePattern (checkbox/switch). UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Expand or collapse an element by ref via UIA ExpandCollapsePattern (tree node / expander / combo). UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Select an element by ref via UIA SelectionItemPattern (list item / radio / tab). Replaces the current selection (no multi-select this phase). UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Scroll an element into view by ref via UIA ScrollItemPattern (realize an item in a scrollable container, then re-snapshot). UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Scroll a container by ref via UIA ScrollPattern. direction = up|down|left|right; amount = number of small scroll steps (1..50). Realize virtualized items, then re-snapshot. UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
    ...
    [McpServerTool(Destructive = true), Description("Transform a window by handle via UIA Window/Transform patterns. action = maximize|minimize|restore. (Close is desktop_close_window; move/resize land later.) UIA pattern call, not synthetic input: NO input lease required. Blocked in --read-only-mode.")]
