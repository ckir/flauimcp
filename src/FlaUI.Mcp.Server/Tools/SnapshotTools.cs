...
public sealed class SnapshotTools
{
    ...
    [McpServerTool(ReadOnly = true), Description("Diff a window's CURRENT tree against an explicit baseline snapshotId. Returns added/removed/changed (Name/Enabled/Focused/Selected) keyed by composite identity (ControlType+AutomationId+RuntimeId, else +Name). Result refs belong to the new currentSnapshotId. Optional scope=<a live ref, typically from the baseline> diffs ONLY that element's subtree (cheap: re-walks just the subtree, slices the baseline in-memory). Note: anonymous virtualized recycled rows (empty AutomationId+Name, recycled RuntimeId) can collide - diff such content by value/text instead.")]
    ...
    [McpServerTool(ReadOnly = true), Description("Poll a window until a selector condition holds. by=automationId|name|controlType, value=target, until=exists|enabled|gone|valueEquals (equals required for valueEquals; compares ValuePattern→Name→LegacyIAccessible). Timeout returns {satisfied:false} (NOT an error). On success returns the matched ref + a fresh snapshotId. Polls are transient (no ref growth). On an exists/enabled timeout where the element resolves but lies outside the window rect, returns unsatisfiedBecause=\"outsideWindowBounds\" plus elementBounds and windowBounds (each [x, y, width, height]) so a geometry failure is not mistaken for a timing one. until=gone splits two kinds of not-visible: an element UIA reports as offscreen still satisfies it instantly, but one merely laid out past the window edge does NOT (it used to, reporting a live element as destroyed). includeOffscreen:true makes gone STRICTER, not looser: it keeps offscreen elements in the tree, so gone then waits for the element to actually LEAVE the tree. Note until=valueEquals never applied the window-edge cull, so it reaches past-the-edge elements even with includeOffscreen:false. BUDGET: each poll is a full tree walk (seconds on a large window) and the budget is only checked BETWEEN polls, so elapsedMs routinely OVERSHOOTS timeoutMs by whole walks — a short timeoutMs degrades into a single-shot check rather than failing fast. until=gone costs one extra confirmation walk before it will report success.")]
